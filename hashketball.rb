...
def num_points_scored(player)
  game_hash.each do |location, info|
    info.each do |location_info, players|
      if location_info == :players
        players.each do |team_player, stats|
...
def team_colors(team_name)
  game_hash.each do |location, info|
      info.each do |location_info, stats|
        if stats == team_name
          return info[:colors]
        end
      end
...
def player_stats(players_name)
  new_array = []
  game_hash.each do |location, info|
    info[:players].each do |name, player_stats|
